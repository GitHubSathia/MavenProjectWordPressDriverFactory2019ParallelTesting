...
public class PostsPage {
	...
	public static String addNewPost(String title,String block) throws InterruptedException {
		...
		blockTextBox.click();
		Actions keydown=new Actions(DriverFactory.getInstance().getDriver());
	...
	public static String modifyPost(String title,String block) {
		String status=null;
		allPostsLink.click();
		//List<WebElement> col=DriverFactory.getInstance().getDriver().findElements(By.xpath("//table[@class='wp-list-table widefat fixed striped posts']/thead/tr/th"));
		//List<WebElement> row=DriverFactory.getInstance().getDriver().findElements(By.xpath("//table[@class='wp-list-table widefat fixed striped posts']/tbody/tr"));
		...
		/*for(int i=0;i<row.size();i++)
		{
			...
			{
				String celltext=DriverFactory.getInstance().getDriver().findElement(By.xpath("//table[@class='wp-list-table widefat fixed striped posts']/tbody/tr[" + (i+1)+ "]/td[" + (j+1)+ "]")).getText();
		...
		for(int i=0;i<row.size();i++)
		{
				//WebElement data=DriverFactory.getInstance().getDriver().findElement(By.xpath("//table[@class='wp-list-table widefat fixed striped posts']/tbody/tr[" + (i+1)+ "]/td[1]"));
				String celltext=data.getText();
				if(celltext.equalsIgnoreCase(title)) {
					Actions mouseover=new Actions(DriverFactory.getInstance().getDriver());
					mouseover.moveToElement(data).build().perform();
					//WebElement edit=DriverFactory.getInstance().getDriver().findElement(By.xpath("//table[@class='wp-list-table widefat fixed striped posts']/tbody/tr[" + (i+1)+ "]/td[1]/div[3]/span[1]"));
					edit.click();
					editTextBox.clear();
					//WebElement editTextBox=DriverFactory.getInstance().getDriver().findElement(By.xpath("//p[contains(@class,'block-editor-rich-text__editable editor')]"));
					//editTextBox.clear();
					Actions keydown1=new Actions(DriverFactory.getInstance().getDriver());
